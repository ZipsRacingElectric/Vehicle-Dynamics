...
MRF = ZR26.motion_ratio_front;
MRR = ZR26.motion_ratio_rear;
...
preTableF = [springRate'];
preTableR = [springRate', repmat(rearARBStiffness', rowForTable, 1)];
wheelRateFTable = nan(rowForTable,1);
wheelRateRTable = nan(rowForTable,1);
...
for i = 1:rowForTable
wheelRateFTable(i,1) = springRate(i) * MRF.^2;
end
for i = 1:rowForTable
wheelRateRTable(i,1) = springRate(i) * MRR.^2;
end
for i = 1:rowForTable
axleStiffnessFTable(i,1) = wheelRateFTable(i) .* frontTrackWidth ./ 2;
end
for i = 1:rowForTable
axleStiffnessRTable(i,1) = wheelRateRTable(i) .* rearTrackWidth ./ 2 + rearARBStiffness;
end
for i = 1:rowForTable
totalRollStiffnessTable(i,1) = axleStiffnessFTable(i) + axleStiffnessRTable(i);
end

tableF = [preTableF, wheelRateFTable, axleStiffnessFTable, totalRollStiffnessTable]
tableR = [preTableR, wheelRateRTable, axleStiffnessRTable, totalRollStiffnessTable]
