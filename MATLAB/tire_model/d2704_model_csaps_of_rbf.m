%{
%% Overview:
This script is a little silly in that it fits a CSAPs to the RBF Fit Brian
made. Why? Can't exactly remember. But it's been sitting in my stashed
changes forever and I want it gone.
I think I was using the cubic spline coeffs for my senior project cause
they were easier to implement than an RBF fit.

-BMM, 10/20 but actually written around 4/26
...
rbf_Fy = rbfcreate(X, Fy', 'RBFFunction', 'multiquadric', 'RBFConstant', sigma, 'RBFSmooth', smooth_param,'Stats','on');
rbf_Fx = rbfcreate(X, Fx', 'RBFFunction', 'multiquadric', 'RBFConstant', sigma, 'RBFSmooth', smooth_param,'Stats','on');
rbf_Mz = rbfcreate(X, Mz', 'RBFFunction', 'multiquadric', 'RBFConstant', sigma, 'RBFSmooth', smooth_param,'Stats','on');

% Define query point for interpolation/extrapolation (must be normalized
SL_q = (0 - min(SL)) ./ (max(SL) - min(SL));  % Example Slip Ratio
SA_q = (5 - min(SA)) ./ (max(SA) - min(SA));     % Example Slip Angle
Fz_q = (-800 - min(Fz)) ./ (max(Fz) - min(Fz));  % Example Normal Load
...
%% Refit data with csaps
SL_norm = (sl_vals - min(SL)) / (max(SL) - min(SL));
SA_norm = (sa_vals - min(SA)) / (max(SA) - min(SA));
Fz_norm = (fz_vals - min(Fz)) / (max(Fz) - min(Fz));


csaps_grid = {sl_vals',sa_vals',fz_vals'};

Fy_poly = csaps(csaps_grid,Fy_grid);
Fx_poly = csaps(csaps_grid,Fx_grid);
Mz_poly = csaps(csaps_grid,Mz_grid);

query_point = [0;5;-800];

Fy_poly_q = fnval(Fy_poly,query_point);
Fx_poly_q = fnval(Fx_poly,query_point);
Mz_poly_q = fnval(Mz_poly,query_point);


fprintf('CSAPS at sample query point:\n Fy = %.2f, Fx = %.2f, Mz = %.2f\n', Fy_poly_q, Fx_poly_q, Mz_poly_q);


for i=1:size(Fy_grid,1)
    for j = 1:size(Fy_grid,2)
        for k = size(Fy_grid,3)
            Fy_grid(i,j,k) = fnval(Fy_poly,[sl_vals(i); sa_vals(j); fz_vals(k)]);
            Fx_grid(i,j,k) = fnval(Fx_poly,[sl_vals(i); sa_vals(j); fz_vals(k)]);
            Mz_grid(i,j,k) = fnval(Mz_poly,[sl_vals(i); sa_vals(j); fz_vals(k)]);
        end
    end
end
