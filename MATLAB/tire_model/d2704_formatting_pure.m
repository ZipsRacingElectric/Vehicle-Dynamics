...
for n=1:2:(length(z)-2) % every 3 zero crossings represents a full SA sweep
    ...
    for slip = floor(min(sa)):1:ceil(max(sa)) % full sweep of slip angle spline in whole integers of degrees
        q = q+1;
        fmdata(q,1) = 0; % all pure cornering data is at SL = 0 
        fmdata(q,2) = slip; 
        fmdata(q,3) = round(mean(ia));
        fmdata(q,4) = round(mean(pressure));
        fmdata(q,5) = mean(fz);
        fmdata(q,6) = fnval(sp_fy,slip);
        fmdata(q,7) = 0; % all pure cornering data is at Fx = 0 
...
% for each distinct P & IA combination, concatenate array of slip
% angles and corresponding 0 Fz data
zero_load = [];
for n = 1:length(press)
    for q = 1:length(incls)
        zero_load = [zero_load; [zeros(length(slips),1), slips, (zeros(length(slips),1)+incls(q)), (zeros(length(slips),1)+press(n)), zeros(length(slips),1), zeros(length(slips),1), zeros(length(slips),1), zeros(length(slips),1), zeros(length(slips),1)]];
    end
end
...
save("../tire_data/d2704/d2704_7in_formatted_data.mat", "fmdata_pure");
