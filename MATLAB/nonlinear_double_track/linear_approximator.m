...
sim('nonlinear_double_track_model');  % Runs the simulation
...
load dataFile
Z = iddata(outData,inData,0.01); %there must be a constant solving rate (fix in model settings)
Z.InputName = 'Reference Yaw Rate';
Z.InputUnit = 'rad/s';

Z.OutputName = {'Yaw Rate'};
Z.OutputUnit = {'rad/s'};
...
subplot(2,1,2);
plot(t,Z.u), ylabel('Reference Yaw Rate (rad/s)')
% axis([0 50 -1.2 1.2])
xlabel('Time (seconds)')
...
clf
compare(Z, syslin2, syslin3, syslin4, syslin5, syslin6)

% Extract state-space matrices from the idss object
A = syslin2.A;  % Extract state matrix
B = syslin2.B;  % Extract input matrix
C = syslin2.C;  % Extract output matrix
D = syslin2.D;  % Extract direct transmission matrix
